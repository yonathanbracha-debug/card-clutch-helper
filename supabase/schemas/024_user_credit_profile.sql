-- User Credit Profile Table
-- Dedicated onboarding + calibration store for credit pathway engine
-- Separate from user_preferences (which is for UI prefs only)
...
CREATE INDEX IF NOT EXISTS idx_user_credit_profile_user_id ON public.user_credit_profile(user_id);
